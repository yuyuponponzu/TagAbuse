...
print(test_X.shape)
test_X = np.c_[test_Ini_y,test_X]
print(test_X.shape)
...
for train, test in kfold.split(X,cat_y):
    ...
    model.compile(optimizer='Adagrad', loss = lossfunc, metrics=['binary_accuracy', P, R, F])
    model.fit(X[train], diff[train], epochs=20, batch_size=32, verbose=1)#, validation_split=0.1, callbacks=[early_stopping])
    model.save('./model_10val/'+h+'.h5')
    print("Model trained and saved")

    #model = load_model('./model_10val/3122.h5')
    scores = model.evaluate(test_X, test_diff, verbose=0)
    print("%s: %.2f%%" % (model.metrics_names[1], scores[1]*100))
    cvscores.append(scores[1] * 100)

    pred = model.predict(test_X)
    print(pred.shape)
    report = classification_report(test_diff, (pred>threshold).astype(int))
    log.write(h+"\n")
