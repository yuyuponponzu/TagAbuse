...
K = 9
for k in range(K):
    csv_file = open("./noc_DATASET_"+str(k+1)+".csv", "r", encoding ="Latin-1")
    ...
    for row in f:
        ...
        for i in range(len(a)-1):
            b = a[i].split('<')
            tag.append(b[1])
        datafile.write(str(row['p1_Id']) + category_delimiter)
        datafile.write(" ".join(tag) + category_delimiter)
        datafile.write(str(row['p1_Title']) + str(row['p1_Body']) + category_delimiter)
